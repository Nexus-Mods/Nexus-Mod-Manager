...
namespace Nexus.Client.Games.DragonsDogma
{
    ...
    /// factories that build game modes for DragonsDogma based games.
	/// </summary>
	public class DragonsDogmaGameModeFactory : IGameModeFactory
	{
		...
		public DragonsDogmaGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
            EnvironmentInfo = p_eifEnvironmentInfo;
			m_gmdGameModeDescriptor = new DragonsDogmaGameModeDescriptor(p_eifEnvironmentInfo);
		}
        ...
        {
			var registryKey = @"HKEY_CURRENT_USER\Software\Valve\Steam\Apps\367500";
            ...
            try
            {
                ...
                if (!Directory.Exists(appPath))
                {
                    Trace.TraceInformation(
                        "DragonsDogma is not installed in standard directory. Checking steam config.vdf...");
                    ...
                    var node =
                        kv.Children[0].Children[0].Children[0].Children.Single(x => x.Name == "apps")
							.Children.Single(x => x.Name == "367500");
                    if (node != null)
                    {
                        appPath = node.Children.Single(x => x.Name == "installdir").Value;
                        if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, @"DDDA.exe")))
                            strValue = appPath;
                    }
                }
                else
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var uniPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 367500", "InstallLocation", null).ToString();
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
            ...
            DragonsDogmaGameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
            p_imsWarning = null;
        ...
        protected DragonsDogmaGameMode InstantiateGameMode(FileUtil p_futFileUtility)
        {
            return new DragonsDogmaGameMode(EnvironmentInfo, p_futFileUtility);
        }
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
            ...
            DragonsDogmaSetupVM vmlSetup = new DragonsDogmaSetupVM(EnvironmentInfo, GameModeDescriptor);
