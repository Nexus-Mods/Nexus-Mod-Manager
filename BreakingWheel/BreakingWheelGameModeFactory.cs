...
namespace Nexus.Client.Games.BreakingWheel
{
	...
	/// factories that build game modes for BreakingWheel based games.
	/// </summary>
	public class BreakingWheelGameModeFactory : IGameModeFactory
	{
		...
		public BreakingWheelGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
			EnvironmentInfo = p_eifEnvironmentInfo;
			m_gmdGameModeDescriptor = new BreakingWheelGameModeDescriptor(p_eifEnvironmentInfo);
		}
		...
		{
            string strValue = SteamInstallationPathDetector.Instance.GetSteamInstallationPath("545890", "Breaking Wheel", @"Main Game\Ellie_Ball_Project.exe");
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
			...
			BreakingWheelGameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
			p_imsWarning = null;
		...
		protected BreakingWheelGameMode InstantiateGameMode(FileUtil p_futFileUtility)
		{
			return new BreakingWheelGameMode(EnvironmentInfo, p_futFileUtility);
		}
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
			...
			BreakingWheelSetupVM vmlSetup = new BreakingWheelSetupVM(EnvironmentInfo, GameModeDescriptor);
