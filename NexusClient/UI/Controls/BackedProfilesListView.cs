...
{
	public partial class BackedProfilesListView : BrightIdeasSoftware.TreeListView
	{
			...
			: base()
		{
			...
			tlcWorksWithSaves.Name = "WorksWithSaves";
		}
		...
		public void SetupColumnParser()
		{
			...
			tlcWorksWithSaves.AspectGetter = delegate (object rowObject)
			{
				string Val = ((ModProfile)rowObject).WorksWithSaves == 0 ? "No" : "Yes";
			...
			{
				int Val = ((ModProfile)rowObject).Version;
				return Val;
			};
		}
		...
		{
			tlcProfileName.ImageGetter = delegate(object rowObject)
			{
				//try
				//{
		...
		public void LoadData(ThreadSafeObservableList<IModProfile> p_tslModProfile)
		{
			...
			//if ((m_tslEnabledProfiles != null) && (m_tslEnabledProfiles.Count > 0))
			//	this.AutoResizeColumns(ColumnHeaderAutoResizeStyle.ColumnContent);
			//else
			//	this.AutoResizeColumns(ColumnHeaderAutoResizeStyle.HeaderSize);
		...
		{
			tlcProfileName.Width = 120;
			tlcActiveMods.Width = 80;
			tlcLastBackedUp.Width = 90;
			tlcShared.Width = 50;
			tlcWorksWithSaves.Width = 110;
			tlcVersion.Width = 50;
