...
namespace Nexus.Client.ModManagement
{
	public partial class ProfileManager : IProfileManager
	{
		...
		public IModProfile ImportProfile(string p_strPath, string p_strLabel, int p_intProfileId, out string p_strErrorMessage)
		{
			...
			try
			{
				IModProfileInfo impProfileInfo = ModRepository.GetProfileData(ModRepository.RemoteGameId, p_intProfileId);

				ZipFile.ExtractToDirectory(p_strPath, strProfilePath);
				if (impProfileInfo.Name == "")
					strProfileName = ImportedProfileName(Path.Combine(strProfilePath, "profile.txt") ?? "Imported");
				else
					strProfileName = WebUtility.HtmlDecode(impProfileInfo.Name);

				string strConcatName = string.Empty;

				int intNewProfile = 1;
				if (m_tslProfiles.Count > 0)
				{
					List<IModProfile> lstNewProfile = m_tslProfiles.Where(x => x.Name.IndexOf(strProfileName) == 0).ToList();
					if ((lstNewProfile != null) && (lstNewProfile.Count > 0))
					{
						if (p_strLabel != "")
							strConcatName = string.Concat(strProfileName + "(" + p_strLabel);
						else
							strConcatName = strProfileName;


						List<Int32> lstID = new List<Int32>();
						foreach (IModProfile imp in m_tslProfiles)
						{
							if (imp.Name.IndexOf(strConcatName, 0, StringComparison.OrdinalIgnoreCase) >= 0)
							{
								string strName = imp.Name.ToLower();
								int intLabelPosition = strName.IndexOf(p_strLabel, 0, StringComparison.OrdinalIgnoreCase) + p_strLabel.Length;
								int intEndPosition = strName.IndexOf(")", intLabelPosition);
								string strCounter = strName.Substring(intLabelPosition, intEndPosition - intLabelPosition);
								int i = 0;
								if (int.TryParse(strCounter, out i))
									lstID.Add(Convert.ToInt32(i));
							}
						}
						if (lstID.Count > 0)
							strProfileName = string.Concat(strProfileName + "(" + p_strLabel + Enumerable.Range(1, lstID.Max() + 1).Except(lstID).Min().ToString() + ")"); //" " + Enumerable.Range(1, lstID.Max() + 1).Except(lstID).Min().ToString();
						else
							strProfileName = strConcatName + "1)";
					}
				}

				ModProfile mprModProfile = null;

				if (impProfileInfo != null)
				{
					if (string.Equals(ModRepository.UserStatus[0], impProfileInfo.AuthorID, StringComparison.InvariantCultureIgnoreCase))
						impProfileInfo.Author = string.Empty;

					mprModProfile = new ModProfile(strId, strProfileName, ModManager.GameMode.ModeId, impProfileInfo.ActiveMods, false, p_intProfileId.ToString(), "", impProfileInfo.Date.ToShortDateString(), impProfileInfo.Public == 1 ? true : false, impProfileInfo.Version.ToString(), impProfileInfo.Author, 0, false);
				}
				else
				{
					Int32 intModCount = ImportedProfileModCount(Path.Combine(strProfilePath, "profile.xml") ?? "Imported");
					mprModProfile = new ModProfile(strId, strProfileName, ModManager.GameMode.ModeId, intModCount, false, "", "", "", false, "", "", 0, false);
				}
			...
			{
				p_strErrorMessage = "There was an error while importing the profile, the profile could not be imported:" + Environment.NewLine + Environment.NewLine + ex.Message;
			}
