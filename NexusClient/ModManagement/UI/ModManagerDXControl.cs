namespace Nexus.Client.ModManagement.UI
{
	...
	public partial class ModManagerDXControl : ManagedFontDockContent, IModManagerView
	{
		...
		private void ModCategoryTree_SortNumberEditRequested(object sender, ModTreeSortNumberEditEventArgs e)
		{
			...
			{
				_gridModListSurface.RefreshData();
				// The editor has already posted the cell value, so force reconciliation
				// against the persisted Sort number even when the displayed value matches.
				_categoryModListSurface?.RefreshData();
			}
			else
			{
		...
		{
			if (_viewModel == null || p_lstMods == null || p_lstMods.Count == 0 || !p_lstMods.All(HasUsableSourceArchive))
				return;
