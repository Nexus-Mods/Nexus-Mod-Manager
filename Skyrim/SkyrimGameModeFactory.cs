...
{
	/// <summary>
	/// The game mode factory that builds <see cref="SkyrimGameMode"/>s.
	/// </summary>
	public class SkyrimGameModeFactory : GamebryoGameModeFactory
	{
		...
		{
			return new SkyrimGameMode(EnvironmentInfo, p_futFileUtility);
		}
			...
			try
			{
				...
				if (steamKey != null)
				{
					...
					if (isSteamInstall)
					{
						...
						var appPath = Path.Combine(steamPath, @"steamapps\common\Skyrim");

						// check if game is installed in the default directory
						if (!Directory.Exists(appPath))
						{
							Trace.TraceInformation(
								"Skyrim is not installed in standard directory. Checking steam config.vdf...");
							...
							if (node != null)
							{
								...
								if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, "SkyrimLauncher.exe")))
									strValue = appPath;
							}
						}
						else
