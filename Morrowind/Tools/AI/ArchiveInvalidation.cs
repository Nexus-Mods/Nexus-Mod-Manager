...
namespace Nexus.Client.Games.Morrowind.Tools.AI
{
    ...
    public class ArchiveInvalidation : ArchiveInvalidationBase
    {
        ...
        /// </summary>
        private const string OLD_AI_BSA = @"..\mwmm\BSARedirection.bsa";
        ...
        public ArchiveInvalidation(MorrowindGameMode p_gmdGameMode)
            : base(p_gmdGameMode)
        {
        }
        ...
        {
            string strPluginsPath = GameMode.PluginDirectory;
            foreach (FileInfo fi in new DirectoryInfo(strPluginsPath).GetFiles("Morrowind.bsa"))
                fi.LastWriteTime = new DateTime(2005, 10, 1);
            foreach (FileInfo fi in new DirectoryInfo(strPluginsPath).GetFiles("DLC*.bsa"))
                fi.LastWriteTime = new DateTime(2005, 10, 1);
            foreach (FileInfo fi in new DirectoryInfo(strPluginsPath).GetFiles("Tribunal.bsa"))
                fi.LastWriteTime = new DateTime(2005, 10, 1);
            foreach (FileInfo fi in new DirectoryInfo(strPluginsPath).GetFiles("Bloodmoon.bsa"))
                fi.LastWriteTime = new DateTime(2005, 10, 1);
            ...
            FileUtil.ForceDelete(Path.Combine(Path.Combine(GameMode.GameModeEnvironmentInfo.InstallationPath, "mwmm"), AI_BSA));
