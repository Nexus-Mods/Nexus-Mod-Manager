...
namespace Nexus.Client.Games.Fallout4VR.Scripting.XmlScript.CPL
{
	/// <summary>
	/// A converter that translates CPL to and from an <see cref="ICondition"/>,
	/// </summary>
	public class Fallout4VRCplConverter : CPLConverter
	{
		/// <summary>
		...
		public Fallout4VRCplConverter(ICplParserFactory p_pftParserFactory)
		...
		protected override ICondition BuildCompositeCondition(ITree p_astCPL)
		{
			switch (p_astCPL.Type)
			{
				case CPLParser.ATLEAST:
					...
					{
						case Fallout4VRCplParser.SKSE_VERSION:
							return new SkseCondition(verVersion);
					}
					break;
			}
		...
		protected override string GenerateCpl(ICondition p_cndCondition)
		{
			...
			{
				SkseCondition nseCondition = (SkseCondition)p_cndCondition;
				return String.Format("skseVersion >= {0}", nseCondition.MinimumVersion.ToString());
			}
