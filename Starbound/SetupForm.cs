...
namespace Nexus.Client.Games.Starbound
{
	...
	{
        private StarboundSetupVM m_vmlViewModel = null;
		...
		public SetupBaseVM ViewModel
		{
			...
			private set
			{
                m_vmlViewModel = value as StarboundSetupVM;
				if (m_vmlViewModel == null)
					throw new ArgumentException("The given view model must be a StarboundSetupVM. Type found: " + value.GetType().FullName);
