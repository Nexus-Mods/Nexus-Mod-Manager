...
namespace Nexus.Client.Games.Starbound
{
    ...
    /// factories that build game modes for Starbound based games.
	/// </summary>
	public class StarboundGameModeFactory : IGameModeFactory
	{
		...
		public StarboundGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
        ...
        public string GetInstallationPath()
        {
            ...
            try
            {
                ...
                if (steamKey != null)
                {
                    ...
                    if (isSteamInstall)
                    {
                        ...
                        var appPath = Path.Combine(steamPath, @"steamapps\common\Starbound");

                        // check if game is installed in the default directory
                        if (!Directory.Exists(appPath))
                        {
                            Trace.TraceInformation(
                                "Starbound is not installed in standard directory. Checking steam config.vdf...");
                            ...
                            if (node != null)
                            {
                                ...
                                if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, "starbound.exe")))
                                    strValue = appPath;
                            }
                        }
                        else
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var uniPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 211820", "InstallLocation", null).ToString();
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
            ...
            StarboundGameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
            p_imsWarning = null;
        ...
        protected StarboundGameMode InstantiateGameMode(FileUtil p_futFileUtility)
        {
            return new StarboundGameMode(EnvironmentInfo, p_futFileUtility);
        }
