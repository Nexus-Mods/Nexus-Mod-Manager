...
namespace Nexus.Client.Games.Fallout4
{
	...
	public class Fallout4GameModeFactory : GamebryoGameModeFactory
	{
		...
		{
			var registryKey = @"HKEY_CURRENT_USER\Software\Valve\Steam\Apps\377160";
			...
			try
			{
				...
				if (steamKey != null)
				{
					...
					if (isSteamInstall)
					{
						...
						var appPath = Path.Combine(steamPath, @"steamapps\common\Fallout4");

						// check if game is installed in the default directory
						if (!Directory.Exists(appPath))
						{
							Trace.TraceInformation(
								"Fallout4 is not installed in standard directory. Checking steam config.vdf...");
							...
							var node =
								kv.Children[0].Children[0].Children[0].Children.Single(x => x.Name == "apps")
									.Children.Single(x => x.Name == "377160");
							if (node != null)
							{
								appPath = node.Children.Single(x => x.Name == "installdir").Value;
								if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, "Fallout4Launcher.exe")))
									strValue = appPath;
							}
						}
						else
