...
namespace Nexus.Client.Games.Grimrock
{
	...
	{
		private GrimrockSetupVM m_vmlViewModel = null;
		...
		public SetupBaseVM ViewModel
		{
			...
			private set
			{
				m_vmlViewModel = value as GrimrockSetupVM;
				if (m_vmlViewModel == null)
					throw new ArgumentException("The given view model must be a GrimrockSetupVM. Type found: " + value.GetType().FullName);
