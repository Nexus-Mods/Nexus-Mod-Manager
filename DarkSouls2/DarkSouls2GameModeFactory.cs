...
namespace Nexus.Client.Games.DarkSouls2
{
	...
	/// factories that build game modes for DarkSouls2 based games.
	/// </summary>
	public class DarkSouls2GameModeFactory : IGameModeFactory
	{
		...
		public DarkSouls2GameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
			EnvironmentInfo = p_eifEnvironmentInfo;
			m_gmdGameModeDescriptor = new DarkSouls2GameModeDescriptor(p_eifEnvironmentInfo);
		}
		...
		{
			var registryKey = @"HKEY_CURRENT_USER\Software\Valve\Steam\Apps\236430";
			...
			try
			{
				...
				if (steamKey != null)
				{
					...
					if (isSteamInstall)
					{
						...
						var appPath = Path.Combine(steamPath, @"steamapps\common\Dark Souls II\Game");

						// check if game is installed in the default directory
						if (!Directory.Exists(appPath))
						{
							Trace.TraceInformation(
								"Dark Souls II is not installed in standard directory. Checking steam config.vdf...");
							...
							var node =
								kv.Children[0].Children[0].Children[0].Children.Single(x => x.Name == "apps")
									.Children.Single(x => x.Name == "236430");
							if (node != null)
							{
								appPath = node.Children.Single(x => x.Name == "installdir").Value;
								if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, "DarkSoulsII.exe")))
									strValue = appPath;
							}
						}
						else
				...
				{
					var strPath = Registry.GetValue(@"HKEY_CURRENT_USER\Software\Valve\Steam", "SteamPath", null).ToString();

					if (!String.IsNullOrEmpty(strPath))
					{
						// convert path to windows path. (steam uses C:/x/y we want C:\\x\\y
						strPath = Path.GetFullPath(strPath);
						var strAppPath = Path.Combine(strPath, @"steamapps\common\Dark Souls II\Game");
						if (Directory.Exists(strAppPath) && File.Exists(Path.Combine(strAppPath, "DarkSoulsII.exe")))
							strValue = strAppPath;
					}
				}
			}
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var uniPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 236430", "InstallLocation", null).ToString();
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
			...
			DarkSouls2GameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
			p_imsWarning = null;
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
			...
			DarkSouls2SetupVM vmlSetup = new DarkSouls2SetupVM(EnvironmentInfo, GameModeDescriptor);
