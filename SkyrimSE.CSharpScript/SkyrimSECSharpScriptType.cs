...
{
	/// <summary>
	/// Describes the SkyrimSE variant of the C# script type.
	/// </summary>
	public class SkyrimSECSharpScriptType : CSharpScriptType
	{
		#region Properties
		...
		protected override Type BaseScriptType
		{
			get
			{
				return typeof(SkyrimSECSharpBaseScript);
			}
		}
		...
		{
			BsaManager bmgBsaManager = new BsaManager((SkyrimSEGameMode)p_gmdGameMode);
			UIUtil uitUiUtilities = new UIUtil(p_gmdGameMode, p_eifEnvironmentInfo, p_scxUIContext);
			return new SkyrimSECSharpScriptFunctionProxy(p_modMod, p_gmdGameMode, p_eifEnvironmentInfo, p_ivaVirtualModActivator, p_igpInstallers, bmgBsaManager, uitUiUtilities);
		}
	}
}
