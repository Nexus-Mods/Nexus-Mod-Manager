...
namespace Nexus.Client.Games.StardewValley
{
	...
	/// factories that build game modes for StardewValley based games.
	/// </summary>
	public class StardewValleyGameModeFactory : IGameModeFactory
	{
...
/// <value>The descriptor of the game mode that this factory builds.</value>
		...
		public StardewValleyGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
			EnvironmentInfo = p_eifEnvironmentInfo;
			m_gmdGameModeDescriptor = new StardewValleyGameModeDescriptor(p_eifEnvironmentInfo);
		}
		...
		{
			string strValue = SteamInstallationPathDetector.Instance.GetSteamInstallationPath("413150", "Stardew Valley", @"Stardew Valley.exe");
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var gogPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\GOG.com\Games\1453375253", "PATH", null)?.ToString();
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
			...
			StardewValleyGameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
			p_imsWarning = null;
		...
		protected StardewValleyGameMode InstantiateGameMode(FileUtil p_futFileUtility)
		{
			return new StardewValleyGameMode(EnvironmentInfo, p_futFileUtility);
		}
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
			...
			StardewValleySetupVM vmlSetup = new StardewValleySetupVM(EnvironmentInfo, GameModeDescriptor);
