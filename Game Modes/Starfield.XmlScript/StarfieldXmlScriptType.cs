...
using Nexus.Client.Games.Starfield.Scripting.XmlScript.CPL;
using Nexus.Client.Games.Starfield.Scripting.XmlScript.Parsers;
using Nexus.Client.Games.Starfield.Scripting.XmlScript.Unparsers;
...
namespace Nexus.Client.Games.Starfield.Scripting.XmlScript
{
	/// <summary>
	/// Describes the Starfield variant of the XML script type.
	/// </summary>
	public class StarfieldXmlScriptType : XmlScriptType
	{
		#region Properties
		...
		protected override string GameSpecificXMLScriptSchemaPath
		{
			get
			{
				return "Nexus/Client/Games/Starfield/Scripting/XmlScript/data";
			}
		}
		...
		protected override IParser GetParser(XElement p_xelScript)
		{
			...
			switch (strScriptVersion)
			{
				...
				case "5.0":
					return new StarfieldParser50(p_xelScript, this);
			}
		...
		protected override IUnparser GetUnparser(Nexus.Client.ModManagement.Scripting.XmlScript.XmlScript p_xscScript)
		{
			switch (p_xscScript.Version.ToString())
			{
				...
				case "5.0":
					return new StarfieldUnparser50(p_xscScript);
		...
		{
			return new StarfieldCplParserFactory();
		}
		...
		{
			return new StarfieldConditionStateManager(p_modMod, p_gmdGameMode, p_pmgPluginManager, p_eifEnvironmentInfo);
		}
	}
}
