...
{
	/// <summary>
	/// Describes the SkyrimGOG variant of the C# script type.
	/// </summary>
	public class SkyrimGOGCSharpScriptType : CSharpScriptType
	{
		#region Properties
		...
		protected override Type BaseScriptType
		{
			get
			{
				return typeof(SkyrimGOGCSharpBaseScript);
			}
		}
		...
		{
			BsaManager bmgBsaManager = new BsaManager((SkyrimGOGGameMode)p_gmdGameMode);
			UIUtil uitUiUtilities = new UIUtil(p_gmdGameMode, p_eifEnvironmentInfo, p_scxUIContext);
			return new SkyrimGOGCSharpScriptFunctionProxy(p_modMod, p_gmdGameMode, p_eifEnvironmentInfo, p_ivaVirtualModActivator, p_igpInstallers, bmgBsaManager, uitUiUtilities);
		}
	}
}
