...
namespace Nexus.Client.Games.DragonAge
{
	...
	public class DragonAgeGameMode : GameModeBase
	{
		...
		public override void ModFileMerge(IList<IMod> p_lstActiveMods, IMod p_modMod, bool p_booRemove)
		{
			...
			#region currentMod
			if (!p_booRemove && (p_modMod != null) && (!p_lstActiveMods.Contains(p_modMod)))
			{
		...
		public override bool CheckSecondaryInstall(IMod p_modMod)
		{
			...
			if (!String.IsNullOrEmpty(strModFileName))
				if (Path.GetExtension(strModFileName).Equals(".dazip", StringComparison.InvariantCultureIgnoreCase))
					return true;
