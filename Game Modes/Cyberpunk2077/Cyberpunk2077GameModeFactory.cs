...
namespace Nexus.Client.Games.Cyberpunk2077
{
	...
	/// factories that build game modes for Cyberpunk2077 based games.
	/// </summary>
	public class Cyberpunk2077GameModeFactory : IGameModeFactory
	{
...
/// <value>The descriptor of the game mode that this factory builds.</value>
		...
		public Cyberpunk2077GameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
		...
		{
            var strValue = SteamInstallationPathDetector.Instance.GetSteamInstallationPath("1091500", "Cyberpunk 2077", "Cyberpunk2077.exe");
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var gogPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\GOG.com\Games\1423049311", "PATH", null)?.ToString();
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
			...
			Cyberpunk2077GameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
			p_imsWarning = null;
		...
		protected Cyberpunk2077GameMode InstantiateGameMode(FileUtil p_futFileUtility)
		{
			return new Cyberpunk2077GameMode(EnvironmentInfo, p_futFileUtility);
		}
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
			...
			Cyberpunk2077SetupVM vmlSetup = new Cyberpunk2077SetupVM(EnvironmentInfo, GameModeDescriptor);
