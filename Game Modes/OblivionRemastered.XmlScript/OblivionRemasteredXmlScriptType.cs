...
using Nexus.Client.Games.OblivionRemastered.Scripting.XmlScript.CPL;
using Nexus.Client.Games.OblivionRemastered.Scripting.XmlScript.Parsers;
using Nexus.Client.Games.OblivionRemastered.Scripting.XmlScript.Unparsers;
...
namespace Nexus.Client.Games.OblivionRemastered.Scripting.XmlScript
{
	/// <summary>
	/// Describes the OblivionRemastered variant of the XML script type.
	/// </summary>
	public class OblivionRemasteredXmlScriptType : XmlScriptType
	{
		#region Properties
		...
		protected override string GameSpecificXMLScriptSchemaPath
		{
			get
			{
				return "Nexus/Client/Games/OblivionRemastered/Scripting/XmlScript/data";
			}
		}
		...
		protected override IParser GetParser(XElement p_xelScript)
		{
			...
			switch (strScriptVersion)
			{
				...
				case "5.0":
					return new OblivionRemasteredParser50(p_xelScript, this);
			}
		...
		protected override IUnparser GetUnparser(Nexus.Client.ModManagement.Scripting.XmlScript.XmlScript p_xscScript)
		{
			switch (p_xscScript.Version.ToString())
			{
				...
				case "5.0":
					return new OblivionRemasteredUnparser50(p_xscScript);
		...
		{
			return new OblivionRemasteredCplParserFactory();
		}
		...
		{
			return new OblivionRemasteredConditionStateManager(p_modMod, p_gmdGameMode, p_pmgPluginManager, p_eifEnvironmentInfo);
		}
	}
}
