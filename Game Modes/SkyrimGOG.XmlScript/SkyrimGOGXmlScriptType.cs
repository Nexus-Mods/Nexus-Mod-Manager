...
using Nexus.Client.Games.SkyrimGOG.Scripting.XmlScript.CPL;
using Nexus.Client.Games.SkyrimGOG.Scripting.XmlScript.Parsers;
using Nexus.Client.Games.SkyrimGOG.Scripting.XmlScript.Unparsers;
...
{
	/// <summary>
	/// Describes the SkyrimGOG variant of the XML script type.
	/// </summary>
	public class SkyrimGOGXmlScriptType : XmlScriptType
	{
		#region Properties
		...
		protected override string GameSpecificXMLScriptSchemaPath
		{
			get
			{
				return "Nexus/Client/Games/SkyrimGOG/Scripting/XmlScript/data";
			}
		}
		...
		protected override IParser GetParser(XElement p_xelScript)
		{
			...
			switch (strScriptVersion)
			{
				...
				case "5.0":
					return new SkyrimGOGParser50(p_xelScript, this);
			}
		...
		protected override IUnparser GetUnparser(Nexus.Client.ModManagement.Scripting.XmlScript.XmlScript p_xscScript)
		{
			switch (p_xscScript.Version.ToString())
			{
				...
				case "5.0":
					return new SkyrimGOGUnparser50(p_xscScript);
		...
		{
			return new SkyrimGOGCplParserFactory();
		}
		...
		{
			return new SkyrimGOGConditionStateManager(p_modMod, p_gmdGameMode, p_pmgPluginManager, p_eifEnvironmentInfo);
		}
	}
}
