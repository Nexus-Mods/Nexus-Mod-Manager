...
{
	/// <summary>
	/// Describes the Starfield variant of the C# script type.
	/// </summary>
	public class StarfieldCSharpScriptType : CSharpScriptType
	{
		#region Properties
		...
		protected override Type BaseScriptType
		{
			get
			{
				return typeof(StarfieldCSharpBaseScript);
			}
		}
		...
		{
			BsaManager bmgBsaManager = new BsaManager((StarfieldGameMode)p_gmdGameMode);
			UIUtil uitUiUtilities = new UIUtil(p_gmdGameMode, p_eifEnvironmentInfo, p_scxUIContext);
			return new StarfieldCSharpScriptFunctionProxy(p_modMod, p_gmdGameMode, p_eifEnvironmentInfo, p_ivaVirtualModActivator, p_igpInstallers, bmgBsaManager, uitUiUtilities);
		}
	}
}
