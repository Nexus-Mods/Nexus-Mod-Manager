...
namespace Nexus.Client.Games.HogwartsLegacy
{
	...
	/// factories that build game modes for HogwartsLegacy based games.
	/// </summary>
	public class HogwartsLegacyGameModeFactory : IGameModeFactory
	{
...
/// <value>The descriptor of the game mode that this factory builds.</value>
		...
		public HogwartsLegacyGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
			EnvironmentInfo = p_eifEnvironmentInfo;
			m_gmdGameModeDescriptor = new HogwartsLegacyGameModeDescriptor(p_eifEnvironmentInfo);
		}
		...
		{
			string strValue = SteamInstallationPathDetector.Instance.GetSteamInstallationPath("990080", "Hogwarts Legacy", @"HogwartsLegacy.exe");
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
			...
			HogwartsLegacyGameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
			p_imsWarning = null;
		...
		protected HogwartsLegacyGameMode InstantiateGameMode(FileUtil p_futFileUtility)
		{
			return new HogwartsLegacyGameMode(EnvironmentInfo, p_futFileUtility);
		}
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
			...
			HogwartsLegacySetupVM vmlSetup = new HogwartsLegacySetupVM(EnvironmentInfo, GameModeDescriptor);
