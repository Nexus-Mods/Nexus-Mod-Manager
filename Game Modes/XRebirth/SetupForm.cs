...
namespace Nexus.Client.Games.XRebirth
{
	...
	{
		private XRebirthSetupVM m_vmlViewModel = null;
		...
		public SetupBaseVM ViewModel
		{
			...
			private set
			{
				m_vmlViewModel = value as XRebirthSetupVM;
				if (m_vmlViewModel == null)
					throw new ArgumentException("The given view model must be a XRebirthSetupVM. Type found: " + value.GetType().FullName);
