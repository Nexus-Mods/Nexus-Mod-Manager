...
using Nexus.Client.Games.Starbound.Settings;
using Nexus.Client.Games.Starbound.Settings.UI;
using Nexus.Client.Games.Starbound.Tools;
...
{
	/// <summary>
	/// Provides information required for the program to manage Starbound game's plugins and mods.
	/// </summary>
    public class StarboundGameMode : GameModeBase
	{
		private StarboundGameModeDescriptor m_gmdGameModeInfo = null;
        private StarboundLauncher m_glnGameLauncher = null;
        private StarboundToolLauncher m_gtlToolLauncher = null;
		...
		/// <summary>
        /// Gets the path to the per user Starbound data.
		/// </summary>
        /// <value>The path to the per user Starbound data.</value>
		...
		public override IGameLauncher GameLauncher
		{
			get
			{
				if (m_glnGameLauncher == null)
                    m_glnGameLauncher = new StarboundLauncher(this, EnvironmentInfo);
		...
		public override IToolLauncher GameToolLauncher
		{
			get
			{
				if (m_gtlToolLauncher == null)
                    m_gtlToolLauncher = new StarboundToolLauncher(this, EnvironmentInfo);
		...
		public StarboundGameMode(IEnvironmentInfo p_eifEnvironmentInfo, FileUtil p_futFileUtility)
		...
		protected override IGameModeDescriptor CreateGameModeDescriptor()
		{
			if (m_gmdGameModeInfo == null)
				m_gmdGameModeInfo = new StarboundGameModeDescriptor(EnvironmentInfo);
