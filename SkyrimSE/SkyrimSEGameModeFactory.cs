...
{
	/// <summary>
	/// The game mode factory that builds <see cref="SkyrimSEGameMode"/>s.
	/// </summary>
	public class SkyrimSEGameModeFactory : GamebryoGameModeFactory
	{
		...
		public SkyrimSEGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
			: base(p_eifEnvironmentInfo)
		{
			m_gmdGameModeDescriptor = new SkyrimSEGameModeDescriptor(p_eifEnvironmentInfo);
		}
		...
		{
			return new SkyrimSEGameMode(EnvironmentInfo, p_futFileUtility);
		}
		...
		{
			var registryKey = @"HKEY_CURRENT_USER\Software\Valve\Steam\Apps\72850";
			...
			try
			{
				...
				if (steamKey != null)
				{
					...
					if (isSteamInstall)
					{
						...
						var appPath = Path.Combine(steamPath, @"steamapps\common\SkyrimSE");

						// check if game is installed in the default directory
						if (!Directory.Exists(appPath))
						{
							Trace.TraceInformation(
								"SkyrimSE is not installed in standard directory. Checking steam config.vdf...");
							...
							var node =
								kv.Children[0].Children[0].Children[0].Children.Single(x => x.Name == "apps")
									.Children.Single(x => x.Name == "72850");
							if (node != null)
							{
								appPath = node.Children.Single(x => x.Name == "installdir").Value;
								if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, "SkyrimSELauncher.exe")))
									strValue = appPath;
							}
						}
						else
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var uniPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 72850", "InstallLocation", null).ToString();
