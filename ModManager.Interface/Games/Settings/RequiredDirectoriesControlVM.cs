...
namespace Nexus.Client.Games.Settings
{
	...
	public class RequiredDirectoriesControlVM : ObservableObject
	{
		...
		protected bool ValidateDirectory(string p_strPath, string p_strPathName, string p_strProperty)
		{
			...
			{
				Errors.SetError(p_strProperty, LanguageManager.Format("GameSettings.Validation.PathCannotMatchProtectedLocations", "You can't set the {0} directory equal to the following:" + Environment.NewLine +
		...
		{
			return ValidateDirectory(ModDirectory, "Mod", ObjectHelper.GetPropertyName(() => ModDirectory));
		}
		...
		{
			return ValidateDirectory(InstallInfoDirectory, "Install Info", ObjectHelper.GetPropertyName(() => InstallInfoDirectory));
		}
		...
		public void LoadSettings()
		{
			...
			if (string.IsNullOrEmpty(ModDirectory))
			{
				...
				{
					string strDefault = Path.Combine(Path.Combine(Path.Combine(Path.Combine(Path.GetPathRoot(strInstallationPath), "Games"), CommonData.ModManagerName), GameModeDescriptor.ModeId), "Mods");
			...
			if (String.IsNullOrEmpty(InstallInfoDirectory))
			{
				...
				if (String.IsNullOrEmpty(strDirectory))
				{
					string strDefault = Path.Combine(Path.Combine(Path.Combine(Path.Combine(Path.GetPathRoot(strInstallationPath), "Games"), CommonData.ModManagerName), GameModeDescriptor.ModeId), "Install Info");
					strDirectory = strDefault;
				}
