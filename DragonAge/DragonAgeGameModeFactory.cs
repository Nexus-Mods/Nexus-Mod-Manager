...
namespace Nexus.Client.Games.DragonAge
{
	...
	/// factories that build game modes for The DragonAge based games.
	/// </summary>
	public class DragonAgeGameModeFactory : IGameModeFactory
	{
		...
		public DragonAgeGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
			EnvironmentInfo = p_eifEnvironmentInfo;
			m_gmdGameModeDescriptor = new DragonAgeGameModeDescriptor(p_eifEnvironmentInfo);
		}
		...
		{
			var registryKey = @"HKEY_CURRENT_USER\Software\Valve\Steam\Apps\17450";
			...
			try
			{
				...
				if (steamKey != null)
				{
					...
					if (isSteamInstall)
					{
						...
						var appPath = Path.Combine(steamPath, @"steamapps\common\dragon age origins");

						// check if game is installed in the default directory
						if (!Directory.Exists(appPath))
						{
							Trace.TraceInformation(
								"Dragon Age Origins is not installed in standard directory. Checking steam config.vdf...");
							...
							var node =
								kv.Children[0].Children[0].Children[0].Children.Single(x => x.Name == "apps")
									.Children.Single(x => x.Name == "17450");
							if (node != null)
							{
								appPath = node.Children.Single(x => x.Name == "installdir").Value;
								if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, "daorigins.exe")))
									strValue = appPath;
							}
						}
						else
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var uniPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 17450", "InstallLocation", null).ToString();
		...
		public string GetInstallationPath(string p_strGameInstallPath)
		{
			...
			strPath = Path.Combine(strPath, @"Bioware\Dragon Age\Packages\Core\Override");
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
			...
			DragonAgeGameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
			p_imsWarning = null;
		...
		protected DragonAgeGameMode InstantiateGameMode(FileUtil p_futFileUtility)
		{
			return new DragonAgeGameMode(EnvironmentInfo, p_futFileUtility);
		}
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
			...
			DragonAgeSetupVM vmlSetup = new DragonAgeSetupVM(EnvironmentInfo, GameModeDescriptor);
