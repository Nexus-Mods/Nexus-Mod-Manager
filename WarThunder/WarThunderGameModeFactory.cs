...
namespace Nexus.Client.Games.WarThunder
{
	...
	/// factories that build game modes for WarThunder based games.
	/// </summary>
	public class WarThunderGameModeFactory : IGameModeFactory
	{
		...
		public WarThunderGameModeFactory(IEnvironmentInfo p_eifEnvironmentInfo)
		{
			EnvironmentInfo = p_eifEnvironmentInfo;
			m_gmdGameModeDescriptor = new WarThunderGameModeDescriptor(p_eifEnvironmentInfo);
		}
		...
		{
			var registryKey = @"HKEY_CURRENT_USER\Software\Valve\Steam\Apps\236390";
			...
			try
			{
				...
				if (steamKey != null)
				{
					...
					if (isSteamInstall)
					{
						...
						var appPath = Path.Combine(steamPath, @"steamapps\common\War Thunder");

						// check if game is installed in the default directory
						if (!Directory.Exists(appPath))
						{
							Trace.TraceInformation(
								"WarThunder is not installed in standard directory. Checking steam config.vdf...");
							...
							var node =
								kv.Children[0].Children[0].Children[0].Children.Single(x => x.Name == "apps")
									.Children.Single(x => x.Name == "236390");
							if (node != null)
							{
								appPath = node.Children.Single(x => x.Name == "installdir").Value;
								if (Directory.Exists(appPath) && File.Exists(Path.Combine(appPath, "aces.exe")))
									strValue = appPath;
							}
						}
						else
			...
			try
			{
				if (string.IsNullOrWhiteSpace(strValue))
				{
					...
					var uniPath = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Steam App 236390", "InstallLocation", null).ToString();
		...
		{
			string strPath = Path.Combine(p_strGameInstallPath, "UserSkins");
			return strPath;
		}
		...
		public IGameMode BuildGameMode(FileUtil p_futFileUtility, out ViewMessage p_imsWarning)
		{
			...
			WarThunderGameMode gmdGameMode = InstantiateGameMode(p_futFileUtility);
			p_imsWarning = null;
		...
		protected WarThunderGameMode InstantiateGameMode(FileUtil p_futFileUtility)
		{
			return new WarThunderGameMode(EnvironmentInfo, p_futFileUtility);
		}
		...
		public bool PerformInitialSetup(ShowViewDelegate p_dlgShowView, ShowMessageDelegate p_dlgShowMessage)
		{
			...
			WarThunderSetupVM vmlSetup = new WarThunderSetupVM(EnvironmentInfo, GameModeDescriptor);
